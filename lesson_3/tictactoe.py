...
def choose_starting_player():
    ...
    while True:
        ...
        if starting_player.lower() == "player":
            return HUMAN_MARKER
        if starting_player.lower() == "computer":
            return COMPUTER_MARKER
        if starting_player.lower() == "choose":
...
def join_or(empty_options, delimiter=", ", word="or"):
    if len(empty_options) > 1:
        return (f"{delimiter.join(empty_options[:len(empty_options) - 1])}"
               f" {word} {empty_options[len(empty_options) - 1]}")
    return empty_options[0]
...
def detect_winner(board):
    for line in WINNING_LINES:
        ...
        if values.count(HUMAN_MARKER) == 3:
            return "Player"
        if values.count(COMPUTER_MARKER) == 3:
            return "Computer"
    return None
